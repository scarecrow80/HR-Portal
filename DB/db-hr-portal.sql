...
CREATE TABLE IF NOT EXISTS `DB_HR_portal`.`Users` (
  ...
  PRIMARY KEY (`idUsers`),
  UNIQUE INDEX `idBrukere_UNIQUE` (`idUsers` ASC))
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `DB_HR_portal`.`Checklist` (
  ...
  UNIQUE INDEX `idSjekkliste_UNIQUE` (`idChecklist` ASC))
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `DB_HR_portal`.`Newemployee` (
  ...
  `international` TINYINT(1) NULL COMMENT 'Norsk/engelsk',
  `startdate` DATE NULL COMMENT 'Ansatt dato',
  PRIMARY KEY (`idNewemployee`),
  UNIQUE INDEX `idNyansatta_UNIQUE` (`idNewemployee` ASC))
ENGINE = InnoDB;
